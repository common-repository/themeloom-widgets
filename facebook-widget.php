...
class ThemeLoom_Facebook_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if($pageinfo) {
			...
			echo "Please check Facebook widget configuration.";
		}
 	...
 	public function form( $instance ) {
		...
		<label for="<?php echo $this->get_field_id('show_likes'); ?>"><?php _e('Show Fan Count','livingos'); ?></label> </p>
        <p>
...
function livingos_get_fb_access_token( $appid, $appsc ) {
	...
	return json_decode($authToken)->access_token;
}
...
function livingos_get_fb_page_info( $page_id, $auth_token, $cache_expire = 21600 ){
	...
	if ( empty( $pageinfo ) ){
		...
		//get page details
		$json_object = livingos_fetchUrl( "https://graph.facebook.com/v2.11/{$page_id}?fields=name,link,fan_count&access_token={$auth_token}" );
...
function livingos_get_fb_page_feed( $page_id, $auth_token, $num_posts, $edge = 'feed', $cache_expire = 21600 ){
	...
	if ( empty( $feedarray ) ){
		...
		//get page feed
		$json_object = livingos_fetchUrl( "https://graph.facebook.com/v2.11/{$page_id}/{$edge}?fields=from,message,name,created_time,description,story,type,link,caption,picture&limit={$num_posts}&access_token={$auth_token}" );
 ...
 */
 function livingos_facebook_feed( $args = array() ) {
	...
	if ( $args['page_title'] ) {
		...
		if ( $args['show_likes'] )
			$output .= "<a href=\"{$pageinfo->link}\" class=\"fb-like-btn\" ><span class=\"fb-likes\">$pageinfo->fan_count</span></a>";
		$output .= "</div>";
	}
	...
	if (!empty($feedarray) && $feedarray->data) {
